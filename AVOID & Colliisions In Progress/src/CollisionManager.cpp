...
bool CollisionManager::leftLineAABBCheck(ship * object1, GameObject * object2)
{
...
bool CollisionManager::centerLineAABBcheck(ship * object1, GameObject * object2)
{
	glm::vec2 lineStart = object1->getPosition(); //pos of ship
	glm::vec2 lineEnd = object1->getPosition() + object1->getCurrentDirection() * 100.0f;
	// aabb
	int boxWidth = object2->getWidth();
	int halfBoxWidth = boxWidth * 0.5f;
	...
	if (!object2->getIsColliding()) {
		...
		switch (object2->getType()) {
		case TARGET:
			object1->setSteeringState(AVOID);
			std::cout << "Collision with Target!" << std::endl;
			TheSoundManager::Instance()->playSound("yay", 0);
			break;
		case OBSTACLE:
