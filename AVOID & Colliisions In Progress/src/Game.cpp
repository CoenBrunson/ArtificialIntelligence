...
Game::Game(): m_debug(false), avoidState(false)
{
}
...
void Game::render()
{
	...
	if (m_debug)
	{
		...
		Util::DrawLine(m_pShip->getPosition(), m_pShip->getPosition() + (m_pShip->getCurrentDirection() - 0.25f) * 150.0f);
		Util::DrawLine(m_pShip->getPosition(), m_pShip->getPosition() + (m_pShip->getCurrentDirection() * 110.0f));
		Util::DrawLine(m_pShip->getPosition(), m_pShip->getPosition() + (m_pShip->getCurrentDirection() + 0.45f) * 75.0f);
	}
...
void Game::update()
{
	...
	Collision::circleAABBCheck(m_pShip, m_pObstacle);
	Collision::leftLineAABBCheck(m_pShip, m_pObstacle);
}
